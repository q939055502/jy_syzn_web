...
**请求信息**
- `GET /api/public/files/download/signed`

**参数说明**
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| file_path | string | 是 | 文件路径，相对于项目根目录，必须严格按照服务器生成的路径使用，不可修改 |
| expire | integer | 是 | 链接过期时间戳（Unix时间），由服务器生成，不可修改 |
| signature | string | 是 | 链接签名，用于验证链接合法性，由服务器生成，不可修改 |
| ip | string | 否 | 限制访问的IP地址（可选），由服务器生成，不可修改 |

**响应说明**
- 成功：返回文件流，Content-Type根据文件类型自动设置，Content-Disposition为attachment，支持浏览器直接下载
- 失败：返回错误信息，常见错误码为403

**成功响应头示例**
```
Content-Disposition: attachment; filename="水泥检测委托单SN-2024-001.docx"
...
**详细说明**：
1. **链接生成机制**
   - 所有下载链接由服务器通过`LinkGeneratorService`生成，使用HMAC-SHA256算法进行签名
   - 链接包含四个核心部分：文件路径、过期时间戳、签名和可选的IP限制
   - 默认过期时间为3600秒（1小时），可通过环境变量`DEFAULT_DOWNLOAD_LINK_EXPIRE`配置

2. **前端正确使用方法**
   - **禁止手动构造下载链接**：前端必须使用服务器返回的完整`download_url`，不可修改任何参数
   - **直接使用完整链接**：将服务器返回的`download_url`直接作为下载地址，可通过`window.open()`或`<a href>`标签直接使用
   - **及时使用链接**：链接具有时效性，建议在获取后立即使用，避免过期
   - **使用示例**：
     ```javascript
     // 正确用法：直接使用服务器返回的完整下载链接
     const downloadLink = response.data[0].download_url;
     window.open(downloadLink, '_blank'); // 在新标签页打开下载
     // 或通过a标签下载
     const a = document.createElement('a');
     a.href = downloadLink;
     a.download = ''; // 文件名由服务器在响应头中指定
...
3. **常见错误原因及解决方案**
   - **链接已过期**：
     - 原因：链接超过了过期时间（默认1小时）
     - 解决：重新调用`GET /api/public/detection/items/{item_id}/templates`接口获取新的下载链接
   - **链接被篡改**：
     - 原因：手动修改了下载链接中的任何参数（如file_path、expire、signature等）
     - 解决：确保前端直接使用服务器返回的完整链接，不做任何修改
   - **IP地址不匹配**：
     - 原因：如果链接生成时指定了IP限制，而实际访问IP与指定IP不符
     - 解决：避免在生成链接时指定IP限制，或确保访问IP与生成链接时使用的IP一致
   - **参数缺失**：
     - 原因：下载链接中缺少必要的参数（file_path、expire或signature）
     - 解决：确保前端使用完整的下载链接，不做任何截断或修改

4. **安全性说明**
   - 无需登录即可访问，但链接带有签名和过期时间，确保了安全性
   - 仅允许访问指定目录下的文件：static/、upload/、files/，防止目录遍历攻击
   - 链接签名使用安全的HMAC-SHA256算法，无法伪造
   - 支持IP限制，可进一步增强安全性

**注意事项**：
- 前端必须使用服务器返回的完整下载链接，不可对链接进行任何修改
- 下载链接具有时效性，建议及时使用
- 如果下载失败，建议重新获取下载链接后再次尝试
- 如有特殊需求需要延长链接有效期，可通过环境变量`DEFAULT_DOWNLOAD_LINK_EXPIRE`配置

**常见问题排查**：
1. 检查下载链接是否完整，包含所有必要参数
2. 检查链接是否过期（可通过`expire`参数值与当前时间戳比较）
3. 确认没有手动修改过链接中的任何参数
4. 如果设置了IP限制，确认访问IP与生成链接时的IP一致
5. 确认文件路径是否正确，文件是否存在于服务器上
