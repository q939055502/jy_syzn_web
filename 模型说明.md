...
本项目采用SQLAlchemy ORM框架实现数据模型，主要分为**检测相关模型**和**用户相关模型**两大模块，分别位于`app/models/detection/`和`app/models/user/`子目录中。
...
__all__ = ['User', 'Category', 'DetectionObject', 'DetectionStandard', 'DetectionItem', 'DetectionParam', 'DelegationFormTemplate', 'DetectionParamStandard']
```
